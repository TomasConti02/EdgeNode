...
class ImageTransformer(Model):
    ...
    #create and conf  the gRPC with the predictor 
    def _get_grpc_stub(self) -> prediction_service_pb2_grpc.PredictionServiceStub:
        if self._grpc_channel is None:
            options = [ ('grpc.max_receive_message_length', 100 * 1024 * 1024),  ('grpc.max_send_message_length', 100 * 1024 * 1024) ] #define the shape of the data exchange by the gRPC contract
    ...
    def preprocess(self, payload: Union[bytes, Any], headers: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        # Estrazione diretta del payload basata sul formato KServe standard
        try:
        ...
        # Fire and forget per salvare l'immagine nel detector
        try:
        ...
        # Decodifica e manipolazione con OpenCV e NumPy
    ...
    async def predict(self, payload: Dict[str, Any], headers: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        # Estrazione diretta senza controlli superflui
        predicted_class = int(response.outputs["predicted_class"].int_val[0])
        
        # Converte i float_val dell'embedding direttamente in lista prendendo i primi 4
